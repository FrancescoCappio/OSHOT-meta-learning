...
class KITTIVOCModelDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, split, desired_classes_subset=None, use_difficult=False, transforms=None):
        ...
        cls = KITTIVOCModelDataset.CLASSES
        ...
        if isinstance(self.image_set, tuple):
            for spl in self.image_set:
                ...
                self.complete_ids_list.extend(KITTIVOCModelDataset._read_image_ids(img_set_split))
        else:
            image_sets_file = os.path.join(self.root, "ImageSets", "Main", "%s.txt" % self.image_set)
            self.complete_ids_list = KITTIVOCModelDataset._read_image_ids(image_sets_file)
    ...
    def map_class_id_to_class_name(self, class_id):
        if self.keep_all_classes:
            return KITTIVOCModelDataset.CLASSES[class_id]
        else:
