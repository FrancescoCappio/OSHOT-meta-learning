...
def do_train_meta(
    ...
    for iteration in range(start_iter, max_iter):
        ...
        #META TASK DA IMPLEMETARE QUA, PASSANDO META=TRUE A MODEL
        optimizer.zero_grad()
        params = None

        for i in range(5):

            aux_loss = model(images, targets=None, auxiliary_task=True, meta = True, params=params)
            if aux_loss == -1:
               continue
        ...
        if not cfg.MODEL.SELF_SUPERVISED or (cfg.MODEL.SELF_SUPERVISED and not cfg.MODEL.SELF_SUPERVISOR.TYPE == "rotation"):
            ...
            #print("SELF SUPERVISED ROTATION TASKKKKKKKKKKKKKKKKKKKK")
            loss_dict = model(images, targets, auxiliary_task=False, params=params)
        ...
        #   print("{} Net Loss : {}".format(k,v.item()))
