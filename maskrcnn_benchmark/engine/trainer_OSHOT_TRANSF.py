...
from ..data.transforms.transforms import DeNormalize, ToPixelDomain, Normalize, RandomChannelsExchange, GaussianBlur
...
def do_train(
    ...
    for iteration in range(start_iter, max_iter):
        ...
        number_of_trasf = 2
        jitter_trasf = transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.1)
        greyScale_trasf = transforms.Grayscale(num_output_channels=3)
        #changeChan_trasf = RandomChannelsExchange()
        #gauss_blur_trasf = GaussianBlur(radius=5)
        ...
        for trasf in range(number_of_trasf):
            ...
            loss_dict = model(image_trasf, targets, auxiliary_task=True)
